    ...
    <div class="container grid px-6 mx-auto">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @forelse($mereks as $merek)
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td class="px-4 py-3">
                                    <span class="font-semibold">{{ $merek->nama_merek }}</span>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-4 text-sm">
                                        ...
                                        <form action="{{ route('merek.destroy', $merek->id) }}" method="POST"
                                            onsubmit="return confirm('Hapus merek ini?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="px-2 py-2 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                        ...
                        @empty
                            <tr>
                                <td colspan="2" class="px-4 py-3 text-center text-gray-500">Data kosong</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div
                ...
                <span class="flex items-center col-span-3">Showing {{ $mereks->firstItem() }}-{{ $mereks->lastItem() }} of
                    {{ $mereks->total() }}</span>
            </div>
        </div>
    </div>
