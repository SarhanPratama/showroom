    ...
    <div class="container grid px-6 mx-auto">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        @forelse($tipes as $tipe)
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td class="px-4 py-3">
                                    <span class="font-semibold">{{ $tipe->nama_tipe }}</span>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-4 text-sm">
                                        ...
                                        <form action="{{ route('tipe.destroy', $tipe->id) }}" method="POST"
                                            onsubmit="return confirm('Hapus tipe ini?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="px-2 py-2 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
            ...
            <div
                ...
                <span class="flex items-center col-span-3">Showing {{ $tipes->firstItem() }}-{{ $tipes->lastItem() }} of
                    {{ $tipes->total() }}</span>
            </div>
        </div>
    </div>
